...
namespace MapMakeData::Layer {
	...
	void LayerData::setSampleData(int mode) {
		...
		if (mode == 1) {
			auto s1 = NodeNumber();
			auto data = LData(u8"山1", texture1, texture1, MapMakeData::TerrainPen::Sub::Key(), false, false, false);
			layerTreeData.newID(s1, u8"LayerImage");
			layerTreeData.add(layerTreeData.rootID, Node<LData>(s1, data));

			auto s2 = NodeNumber();
			data = LData(u8"平地1", texture1, texture1, MapMakeData::TerrainPen::Sub::Key(), false, false, false);
			layerTreeData.newID(s2, u8"関東平地の情報");
			layerTreeData.add(layerTreeData.rootID, Node<LData>(s2, data));
			auto s3 = NodeNumber();
			data = LData(u8"平地2", texture1, texture1, MapMakeData::TerrainPen::Sub::Key(), false, false, false);
			...
			layerTreeData.add(layerTreeData.rootID, Node<LData>(s2, data));
			auto s3 = NodeNumber();
			data = LData(u8"平地1-1", texture1, texture1, MapMakeData::TerrainPen::Sub::Key(), false, false, false);
			layerTreeData.newID(s3, u8"千葉の情報");
			layerTreeData.add(s2, Node<LData>(s3, data));
			auto s4 = NodeNumber();
			data = LData(u8"平地1-1-1", texture1, texture1, MapMakeData::TerrainPen::Sub::Key(), false, false, false);
			...
			auto s6 = NodeNumber();
			data = LData(u8"平地2-1", texture1, texture1, MapMakeData::TerrainPen::Sub::Key(), false, false, false);
			layerTreeData.newID(s6, u8"LayerImage");
			layerTreeData.add(s5, Node<LData>(s6, data));
			auto s7 = NodeNumber();
			data = LData(u8"平地2-2", texture1, texture1, MapMakeData::TerrainPen::Sub::Key(), false, false, false);
